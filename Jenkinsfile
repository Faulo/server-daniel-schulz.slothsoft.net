				...
				script {
					...
					stage ('Deploy stack') {
						dir("/var/vhosts/daniel-schulz") {
							...
							def service = "daniel-schulz_daniel-schulz"
							sh "docker stack deploy daniel-schulz --detach=true --prune --resolve-image=never -c=docker-compose.yml"
